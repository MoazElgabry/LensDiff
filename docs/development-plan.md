...
- Current backend status:
  ...
  - Diagnostics: `LENSDIFF_LOG` enables backend and fallback logging, `LENSDIFF_TIMING` adds per-render timing, and either toggle now also appends diagnostics to a plugin-owned log file (`%LOCALAPPDATA%\LensDiff\Logs\LensDiff.log` on Windows, `~/Library/Logs/LensDiff.log` on macOS) with optional override via `LENSDIFF_LOG_FILE`
...
- Symptom: On macOS Resolve, LensDiff could appear to freeze as soon as the Metal path engaged, while normal LensDiff render logs never appeared.
- Cause: The Metal backend submits many short-lived command buffers and waits synchronously for each one; doing that work on the host-provided Resolve queue risks stalling host-managed scheduling instead of just stalling LensDiff's own work.
- Rule: Keep LensDiff's internal Metal compute work on a plugin-owned command queue for the same `MTLDevice`, and treat the host-provided queue primarily as the device handoff rather than the queue for every blocking internal stage.
- Verification: `LensDiffMetal.mm` now creates and reuses a plugin-owned work queue per Metal device before running PSF-bank, FFT, preserve, field-zone, and output command buffers.

- Symptom: On macOS Resolve, repeated LensDiff renders could make the Resolve process memory climb dramatically, reaching tens or even hundreds of GB during tester sessions.
- Cause: The Apple Objective-C and Objective-C++ sources were not explicitly built with ARC even though the Metal path allocates many owned `newBuffer`, `newLibrary`, pipeline, queue, and related objects per render; the render entry also lacked a local autorelease pool for temporary Objective-C objects such as array literals and strings.
- Rule: Build the Apple `.m` and `.mm` sources with ARC and wrap the Metal render entry in an explicit `@autoreleasepool` so owned Metal objects are reclaimed normally and per-render temporaries are drained promptly even if the host thread does not provide its own pool.
- Verification: `source/CMakeLists.txt` now enables `-fobjc-arc` for Apple Objective-C and Objective-C++ compilation, and `RunLensDiffMetal(...)` in `LensDiffMetal.mm` now executes inside an `@autoreleasepool`.

- Symptom: LensDiff could still feel heavier on macOS than simpler OFX Metal plugins even after queue and memory-lifetime fixes, especially when Resolve requested repeated renders or interactive updates.
- Cause: `LensDiff.cpp` held the plugin's shared PSF/debug cache mutex across the entire render, so all backend execution and GPU waits were serialized behind a cache-protection lock instead of only the cache snapshot/update seams.
- Rule: Treat the shared PSF/debug cache as a short critical section: snapshot it before render, run the backend on a local copy, then publish the updated cache back after a successful render instead of locking the whole render path.
- Verification: `LensDiff.cpp` now copies `cache_` into a render-local `LensDiffPsfBankCache`, runs CUDA/Metal/CPU against that local cache, and only re-enters `cacheMutex_` to publish the updated cache after a successful render.
...
- 2026-04-20
  - Changed the macOS Metal backend to submit LensDiff's internal command buffers through a plugin-owned `MTLCommandQueue` on the host-provided device instead of Resolve's queue directly.
  - Targeted the first-render Resolve freeze reported by the mac tester, where the plugin could hang before the normal LensDiff host-wrapper logs were emitted.
  - Kept the existing shared-buffer/device contract unchanged so the backend still uses the host-provided Metal buffers while avoiding host-queue contention during blocking internal stages.
  - Batched the early Metal decode, working-scale resample, and prepare passes into a single command buffer so ordinary renders stop paying separate submit-and-wait costs for each of those dependent stages.
  - Added a staged Metal render context plus reusable per-render FFT scratch/transposes/temp stacks so the hot render path can encode source spectra, kernel spectra, convolution, field-zone accumulation, preserve, scatter, fringe, final composite, and output packing into a small fixed set of command buffers instead of forcing helper-level waits.
  - Added the dev-only `LENSDIFF_METAL_LEGACY_SYNC=1` escape hatch and extended `LENSDIFF_TIMING` notes with command-buffer count, wait count, and field-zone batch depth so CUDA-parity investigation can flip back to the old sync-heavy ordering without code changes.
  - Enabled ARC for the Apple Objective-C and Objective-C++ sources and wrapped `RunLensDiffMetal(...)` in an explicit `@autoreleasepool` after a mac tester reported Resolve memory ballooning into triple-digit GB during repeated LensDiff renders.
  - Compared LensDiff's macOS structure against Chromaspace and matched the key host-responsiveness pattern there by shrinking LensDiff's shared cache lock to a snapshot/publish seam instead of holding it across the entire render path.
  - Reworked Metal PSF-bank finalize so each wavelength now uses two synchronized GPU batches with on-device normalization, while support-radius estimation still reads back only the ring summaries needed to preserve the existing visual contract.
  - Added reusable PSF-build scratch buffers plus stacked field-zone convolution helpers, new stack-preserving Metal kernels, and canonical 3x3 zone batching so non-legacy Metal field renders can avoid per-zone waits without changing zone weighting or split-mode combine order.
  - Fixed the split-mode Metal render paths so the effect output always builds the required core and structure intermediates even when those auxiliary outputs are not separately requested by the host.
- 2026-04-21
  - Reworked the CUDA PSF-bank path so the shifted raw PSF stays device-resident through finalize, the per-wavelength base/mean/shaped/structure kernels are normalized on device, and only the final cropped cache kernels are read back to host memory.
  - Added reusable CUDA PSF scratch state plus per-render cuFFT plan, source-spectrum, and kernel-spectrum caches so ordinary CUDA renders stop recreating plans and spectra for the same buffers and kernels inside one frame.
  - Added new non-legacy stacked CUDA field paths for canonical 3x3 field renders: non-split spectral now reuses one driver spectrum and maps RGB once after weighted plane accumulation, while mono and split/core/structure modes now keep zone-local ordering through stacked kernels before the final weighted accumulation.
  - Batched the 3x3 CUDA field-zone cache rebuild so the nine zone raw PSFs share one batched cuFFT execution before each zone finalizes through the same cached kernel/shaping contract.
  - Removed the preserve-mode host scalar readback from the normal CUDA render path by switching energy scaling to device-side reductions and device-side scale application.
  - Added `LENSDIFF_CUDA_LEGACY_PIPELINE=1` plus CUDA timing notes for PSF finalize, support-radius readback, crop-normalize, source FFT, kernel FFT, field-zone stack build, stack convolution, and weighted accumulation so parity checks can flip back to the older path without code edits.
  - Routed both the host-wrapper `LENSDIFF_LOG` output and the shared `LENSDIFF_TIMING` stage logs into a plugin-owned append-only diagnostics file so testers can send one stable log artifact without needing terminal capture; added `LENSDIFF_LOG_FILE` as an optional path override.
  - Added reusable CUDA scratch buffers for stacked field-zone temp spectra, plane stacks, weighted spectral planes, and RGB stack intermediates so the canonical 3x3 field path stops reallocating the same large temporaries across full/core/structure passes inside one render.
  - Removed the mono field-path hotspot where the same RGB source spectra were copied nine times into a 27-slice stack; the CUDA path now caches one RGB triplet spectrum stack and broadcasts it across the zone stack before the existing stacked convolution.
  - Added a process-local persistent CUDA kernel-spectrum cache keyed by device, FFT size, kernel size, and kernel-value hash so repeated identical renders can reuse previously built kernel FFTs across frames instead of missing five spectra every render.
  - Extended CUDA timing output with the previously missing hot-path stages: total convolution, scatter, creative fringe, native-resolution resample, composite/output packing, and final packed-output copy.
- Added an adaptive CUDA field-phase execution guard so oversized canonical 3x3 stacked field renders no longer try to materialize giant `(zone x bin)` FFT stacks at 4K-class sizes; large non-split spectral field renders now switch to a microbatched plane-accumulation path that preserves the same weighting and final spectral map without the runaway stack memory cost.
- Oversized stacked mono/split field renders now avoid the giant stacked path too and drop back to the older per-zone cached execution path instead of trying to allocate field stacks that exceed the configured working-set budget.
- Fixed a Resolve playback correctness bug in the CUDA field-phase compositor: the 3x3 zone weights now normalize against a stable full-frame bounds rect from the OFX host request plus absolute tile origins, instead of the current tile size alone, so changing playback ROIs no longer makes field looks drift frame to frame.
- Reworked the CUDA field investigation layer so `LENSDIFF_CUDA_FIELD_VALIDATE=1` can render the active fast field branch and the legacy per-zone branch inside the same request, then log per-output hashes and max-absolute-error diagnostics for `effect`, `core`, and `structure`.
- Added `LENSDIFF_CUDA_FIELD_WEIGHT_SPACE=working|frame` for CUDA field diagnostics, but restored stable frame-bounds weighting as the default because working-image normalization makes field looks depend on the host's current ROI/tile shape during playback; `working` is kept only as an explicit parity/debug override.
- The newer stacked CUDA field branches are now dev-only behind `LENSDIFF_CUDA_FIELD_STACKED=1`. Playback logs showed host-size-dependent branch switching (`legacy` at full frame, `stacked-spectral` on smaller requests) and the stacked spectral path was still far from legacy parity on those smaller requests, which can make field colors appear to change frame to frame during playback even on one backend.
- The batched CUDA 3x3 field-zone cache builder is now also dev-only behind `LENSDIFF_CUDA_BATCHED_FIELD_CACHE=1`; the default path goes back to the older per-zone field-cache build from the stable tree so field correctness no longer depends on the newer batched cache-finalize path.
- The process-wide persistent CUDA kernel-spectrum cache is now dev-only behind `LENSDIFF_CUDA_PERSISTENT_KERNEL_CACHE=1`. The previous default shared FFT spectra across overlapping renders without any explicit cross-stream completion handoff, which made it too risky as a default in Resolve playback even when the per-render local cache was still present.
- Restored the default CUDA legacy field compositor to the old tree's width/height-only weighting kernel instead of the newer frame/tile-origin-aware accumulator. That newer accumulator was still being used inside the supposed legacy branch, so the active field path had not actually matched the older stable implementation yet.
- The persistent per-device cuFFT plan/workspace repository is now also dev-only behind `LENSDIFF_CUDA_PERSISTENT_PLANS=1`. The older stable tree created standalone plans per render; keeping shared plan handles and shared work areas alive across playback renders was too risky as a default while chasing a frame-to-frame field flicker bug.
- Restored the default CUDA field renderer more literally to the old stable tree by adding a dedicated per-zone `renderFromBins` path that uses fresh cuFFT plans and no render-local FFT/spectrum caches for field-zone execution. Earlier rollbacks still left field mode running through the newer cached renderer, so this change removes that remaining modernization from the default field path.
- Re-enabled the safer part of the CUDA optimizations by default only for non-field renders: ordinary global-path renders now use the persistent cuFFT plan repository and the persistent kernel-spectrum cache again, while field-phase renders stay on the conservative stable path. The persistent kernel cache now records a CUDA event on build and waits on that event before reuse, so cross-stream non-field reuse no longer depends on implicit completion timing.
- Added the missing completion fence for shared cuFFT plans too: cached plan handles/workspaces now record a CUDA event when a lease is released and later renders wait on that event before reuse. Without that fence, split-mode playback could reuse a plan/workspace while the prior stream was still finishing async FFT work, which matches the kind of subtle size/softness flicker seen in the split look.
- Split-mode playback logs did not show Resolve alternating smaller buffers: `renderWindow`, `srcBounds`, `dstBounds`, and `frameBounds` stayed full-size while the split flicker persisted. The default global split renderer is now routed through the same conservative old-style `renderFromBins` path used for field stability, while the faster optimized global renderer remains active for the normal physical non-field path.
- Replaced the render-local CUDA FFT cache with a persistent per-device cuFFT plan repository that uses caller-owned work areas via `cufftCreate` / `cufftSetAutoAllocation(0)` / `cufftMakePlanMany` / `cufftSetWorkArea`, so repeated field playback can reuse large batched plans without recreating their temporary work buffers every frame.
- Extended CUDA timing/log output with `frameBounds`, field/PSF key hashes, selected field branch, scratch-estimate bytes, peak cuFFT work-area bytes, and field-validation error stats so the next playback captures can distinguish correctness drift from workspace churn and branch-selection changes.
- Replaced the oversized non-split spectral field fallback with a bounded tiled CUDA path that builds tile-local source FFTs, reuses chunk-local field kernel stacks keyed by tile FFT size plus zone range, and accumulates weighted spectral planes tile-by-tile before one final spectral map; this keeps the existing 3x3 weighting/math contract while removing the default dependency on full-frame `zone x bin` FFT stacks for large field renders.
- The first tiled spectral CUDA field rollout proved both too slow and visibly wrong in validation (`effectMaxAbs ~0.47` versus legacy), so the branch is now kept behind the dev-only `LENSDIFF_CUDA_FIELD_TILED=1` toggle instead of being used by default.
- Guarded the shared OFX-side PSF/field cache publish path with a render-generation check so an older overlapping playback render can no longer finish late and overwrite a newer cache snapshot, which was a plausible source of frame-to-frame field look drift even when the params themselves were static.
- 2026-04-18
  ...
  - Replaced the hot Metal image-convolution path with FFT-driven convolution built on the same GPU complex-buffer primitives, keeping the existing PSF/kernel contract and output-domain crop so the look stays aligned while large-kernel renders stop paying direct spatial-convolution cost.
  - Batched the hot Metal RGB and spectral FFT work more aggressively, added a per-render Metal kernel-spectrum cache keyed by kernel identity plus padded FFT size, and cut repeated command-buffer waits out of the most expensive repeated source-spectrum setup paths without changing the kernel math contract.
  - Added per-render Metal source-spectrum caches keyed by source buffer and padded FFT size, moved preserve-mode energy reduction and scaling fully onto the GPU, and grouped field-zone/native-resample dispatches into fewer command-buffer commits to improve node-interaction responsiveness without changing the evaluated look.
  - Switched Metal startup to prefer a bundled precompiled `.metallib` generated at build time from the existing shader source block, while keeping runtime source compilation as a fallback if the resource is missing.
  - Added Metal render-stage timing breakdowns for PSF-bank preparation, source FFTs, kernel FFT/cache activity, convolution, field-zone processing, and composite/output so performance work on Apple silicon can be guided by actual footage timings instead of guesswork.
...
- Reverted the host-side cache snapshot/publish concurrency experiment and restored the older per-instance render lock behavior from the stable tree, because same-instance overlapping renders remained a plausible source of playback drift and the snapshot path also added avoidable cache-copy cost on non-field renders.
- Applied the same backend-separation lesson from the stabilized CUDA path to Metal: the newer staged/optimized Metal renderer now stays default only for ordinary non-field physical renders, while field-phase, split look, and resolution-aware requests fall back to the conservative sync-heavy path unless explicitly re-enabled with `LENSDIFF_METAL_FAST_FIELD=1`, `LENSDIFF_METAL_FAST_SPLIT=1`, or `LENSDIFF_METAL_FAST_RESOLUTION_AWARE=1`.
- Extended the Metal timing note to log whether the backend chose the `fast` or `stable` path plus the request traits that caused that choice, so mac tester captures can distinguish true leaks from aggressive-path misuse.
- Disabled OFX tile support for the plugin descriptor and source/output clips. The FFT diffraction path is not tile-safe because the result depends on neighboring pixels and stable frame geometry; tester logs showed Resolve alternating full-frame renders with tiny 184x92 requests, which can make brightness appear to drift and can amplify Metal allocation churn during playback.
- Vendored `VkFFT` under `source/external/VkFFT` and wired the Apple build to compile a new `LensDiffMetalVkFFT.mm` wrapper with `VKFFT_BACKEND=5`, `metal-cpp`, and `QuartzCore` linked explicitly.
- Replaced LensDiff's low-level power-of-two square Metal FFT helpers with a cached VkFFT-backed path keyed by `(device, queue, size, batchCount)`, while keeping the older custom row/transpose FFT as the automatic fallback if VkFFT init or append fails.
- Kept the migration intentionally narrow: higher-level Metal spectrum building, convolution, PSF shaping, and composite logic still use the same LensDiff contracts, so the first VkFFT landing changes the FFT engine without reordering the plugin's image math.
- Added `LENSDIFF_METAL_DISABLE_VKFFT=1` as the immediate escape hatch for mac testing, so the tester can flip back to the pre-VkFFT custom FFT engine without rolling back the rest of the Metal backend changes.
- Aligned Metal FFT normalization with CUDA: Metal VkFFT now uses the same unnormalized C2C inverse contract as CUDA VkFFT/cuFFT, the custom Metal fallback no longer normalizes inside the inverse FFT helper, and real-plane extraction applies the single `1 / paddedArea` convolution scale shared by both GPU backends.
- Moved the common large shared-buffer allocation path toward Apple’s heap guidance by backing big transient Metal buffers with reusable `MTLHeap` allocations when available, while still falling back to plain `newBufferWithLength` if heap allocation fails or is explicitly disabled with `LENSDIFF_METAL_DISABLE_HEAPS=1`.
- Extended the Metal timing note to include whether the active render was running with `vkfft` and `heaps` enabled, so the dedicated LensDiff diagnostics file remains enough to tell the tester exactly which Metal runtime path produced a timing capture.
- Registered `source/external/VkFFT` as a real git submodule and updated the macOS Actions checkout step to fetch submodules recursively, so VkFFT is present by default in CI instead of existing only in local developer clones.
- Added a CUDA dual-FFT seam so LensDiff can request `VkFFT` by default while still forcing `cuFFT` through `LENSDIFF_CUDA_FFT_BACKEND=cufft`, with strict-mode fallback diagnostics via `LENSDIFF_CUDA_VKFFT_STRICT=1`.
- Kept the first CUDA VkFFT migration intentionally `C2C`-only to match LensDiff's existing complex-buffer pipeline and reduce parity risk; `R2C/C2R` remains a deliberate second-phase performance investigation once VkFFT stability/parity is proven, because it may unlock worthwhile memory-traffic and throughput gains but is too invasive for the first landing.
- Kept the `Enable Phase` master toggle hidden in Simple mode with the rest of the phase controls; phase remains available in Full mode where the full phase control tree is visible.
- The first real-world CUDA VkFFT field tests showed the risky part is not the ordinary non-field global path but the PSF-bank and conservative field/split paths, which were still trying to use standalone VkFFT plans. Those sensitive paths now explicitly prefer `cuFFT` again by policy while the ordinary non-field render path keeps the default requested `VkFFT` engine.
- Reworked the CUDA FFT selection into subsystem-aware policy resolution for `GlobalRender`, `PsfBank`, and `FieldRender`, with new development toggles for global policy and per-subsystem backend overrides so VkFFT-first and legacy cuFFT-oriented execution remain directly comparable in one build.
- Partitioned VkFFT plan reuse by subsystem instead of sharing one generic repository, separated non-field and field spectrum/kernel cache namespaces by backend, and moved PSF raw-FFT scratch/state into the reusable PSF build context so the VkFFT path stops behaving like a one-off cuFFT-era bolt-on in PSF-heavy and field-heavy code.
- Redesigned the conservative stable renderer path to use cached spectra and persistent subsystem-selected FFT plans instead of direct one-off plan/spectrum construction, so split/field-sensitive execution can exercise VkFFT with the same conservative math/order while still preserving a clean policy fallback to the legacy cuFFT-oriented behavior.
- The first post-redesign split/field playback check showed that making the conservative path cache/policy-aware reintroduced the exact kind of color drift and shape flicker those modes are most sensitive to. Split mode and the legacy field renderer are now routed back through a direct cuFFT-style stable path with one-off spectra/plans, while the broader VkFFT-first redesign remains in place for the ordinary global path and the shared subsystem policy infrastructure.
- Refined that rollback into a VkFFT-compatible design: split mode and the legacy field renderer still avoid the risky cached-spectrum path, but their direct stable execution now resolves the FFT backend through the subsystem policy and can use persistent VkFFT apps underneath when requested. The intended long-term rule is now "direct conservative math path, backend-selectable FFT engine" rather than "conservative path means cuFFT only."
